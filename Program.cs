...
public class Program
{
    public static void Main()
    {
        ...
        group.RemoveUnsuccessfulStudents();
        group.DisplayStudents();
        group.RemoveLeastSuccessfulStudent();
        group.DisplayStudents();
        Random rand = new Random(DateTime.Now.Second);
        group.AddStudent(new Student(Globals.names[rand.Next(Globals.names.Length)], Globals.surnames[rand.Next(Globals.surnames.Length)], rand.Next(18, 23), $"{Globals.streets[rand.Next(Globals.streets.Length)]}, {rand.Next(1, 99)}", $"+380{rand.Next(111111111, 999999999)}", rand.Next(0, 100)));
        Group group2 = new Group(new List<Student>(), "Group 2", "Specialization 2", 2);
        group.TransferStudent(group2, group.students[0]);
        group.DisplayStudents();
        group2.DisplayStudents();
        group2.EditGroupData();
    }
}